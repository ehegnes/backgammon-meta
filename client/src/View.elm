...
view : Model -> Html Msg
view model = viewBoard model

viewBoard : Model -> Html Msg
viewBoard model =
    let custom = style [ ("width", "810px")
                       , ("height", "180px")
...
translate (Pos n col state container)
    = let color = case col of
              ...
              LBlue -> "blue"
      in
      let custom = style [ ("height", "180px")
                         , ("width", "90px")
                         , ("float", "left")
                         , ("background-color", color)] in
      let click = case col of
                      LBlue -> TurnOff
                      _     -> if state == Ready
                               then SendR n -- will probably need a number
                               else ChangeR n
      in
         div [custom, onClick click] []
    
sect : Color -> Int -> Html Msg
sect col n = div [ onClick (ChangeR n) ] []

sectReset : String -> Html Msg
sectReset str = div [ onClick (SendR 1) ] []
...
transit xs =
    case xs of
        ...
        (x :: xs) ->
            case x of
                (Pos n LBlue Ready NoPiece) -> SendReset
                r                 -> transit xs
